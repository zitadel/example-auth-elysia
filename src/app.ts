...
import type { AuthUser } from '@zitadel/elysia-auth';
import { authHandler, getAuthUser, initAuthConfig } from '@zitadel/elysia-auth';
import config from './config.js';
import { getMessage } from './auth/message.js';
import { authConfig, buildLogoutUrl } from './auth/index.js';
import { requireAuth } from './auth/guard.js';
import { viewPlugin } from './middleware/hbs.js';
...
/**
 * Creates and configures the Elysia application with all routes and plugins.
 *
 * @returns Promise resolving to the configured Elysia application instance
 */
export async function build() {
  const app = new Elysia({ adapter: node() })
     ...
     */
    .post('/auth/logout', async ({ request, cookie, redirect }) => {
      const authUser: AuthUser | null = await getAuthUser(request, authConfig);
     ...
     */
    .get('/auth/logout/callback', ({ query, cookie, set }) => {
      const state: string | undefined = query.state;
      const logoutStateCookie = cookie.logout_state?.value as
        | string
        | undefined;
      ...
      if (state && logoutStateCookie && state === logoutStateCookie) {
        ...
        cookie.logout_state.path = '/auth/logout/callback';
        cookie.logout_state.remove();
        set.status = 302;
        set.headers['Location'] = '/auth/logout/success';
      } else {
        const reason: string = encodeURIComponent(
          'Invalid or missing state parameter.',
        );
        set.status = 302;
        set.headers['Location'] = `/auth/logout/error?reason=${reason}`;
      }
    })
     ...
     * being redirected from protected routes via the requireAuth guard.
     *
    ...
    .get('/auth/login', ({ query, view }) => {
      ...
      const providers: Provider[] = authConfig.providers.map((provider) => {
        ...
        return {
          ...
          signinUrl: `/auth/signin/${providerConfig.id}`,
        };
      });
     ...
     */
    .get('/auth/error', ({ query, view }) => {
      const error = query.error;
      const { heading, message } = getMessage(error, 'auth-error');
      return view('auth/error', { heading, message });
    })
      ...
      async ({ request }) => {
        ...
        try {
          ...
          if (!idpRes.ok) {
            return new Response(
              JSON.stringify({
                error: `UserInfo API error: ${idpRes.status}`,
              }),
              {
    ...
    .get('/', async ({ request, view }) => {
      const session = await getAuthUser(request, authConfig);
      return view('index', {
     ...
     */
    .get('/auth/logout/error', ({ query, view }) => {
      const reason = query.reason ?? 'An unknown error occurred.';
      return view('auth/logout/error', { reason });
    })

    /**
     * Mounts Auth.js Elysia plugin to handle OAuth 2.0/OIDC authentication flows.
     *
     * This plugin provides the complete authentication infrastructure including
     ...
     *
     * The authHandler plugin registers several endpoints for authentication:
     ...
     * plugin to prevent conflicts. Elysia matches routes in definition order,
     * and this plugin will intercept ALL `/auth/*` requests that don't match
     ...
     * plugin throws an UnknownAction error. This happens when:
     * - Custom auth routes are defined after this plugin
     * - Invalid or misspelled auth endpoints are accessed
     * - Routes conflict with Auth.js internal naming conventions
     *
     * The plugin behavior is controlled by the `authConfig` object which
