...
[Elysia](https://elysiajs.com/) is an ergonomic web framework for building backend servers with Bun and Node.js. It provides end-to-end type safety, a plugin-based architecture, and excellent developer experience. In a modern setup, your Elysia application manages both your application's frontend and backend logic through server-side routes and plugins.

To secure such an application, you need a reliable way to handle user logins. For the Elysia ecosystem, [Auth.js](https://authjs.dev/) (formerly NextAuth.js) is the standard and recommended library for authentication. Think of it as a flexible security guard for your app. This guide demonstrates how to use Auth.js with an Elysia application to implement a secure login with ZITADEL.
...
This example uses **Auth.js**, the standard for Elysia authentication. While ZITADEL doesn't offer a specific SDK, Auth.js is highly modular. It works with a "provider" that handles the communication with ZITADEL. Under the hood, this example uses the powerful OIDC standard to manage the secure PKCE flow.

Check out our Example Application to see it in action.

## Example Application

The example repository includes a complete Elysia application, ready to run, that demonstrates how to integrate ZITADEL for user authentication.

This example application showcases a typical web app authentication pattern: users start on a public landing page, click a login button to authenticate with ZITADEL, and are then redirected to a protected profile page displaying their user information. The app also includes secure logout functionality that clears the session and redirects users back to ZITADEL's logout endpoint. All protected routes are automatically secured using Auth.js plugins and session management, ensuring only authenticated users can access sensitive areas of your application.
...
>
> These URLs must exactly match what your Elysia application uses. For production, add your production URLs.
...
```dotenv
# Port number where your Elysia server will listen for incoming HTTP requests.
...
git clone git@github.com:zitadel/example-auth-elysia.git

cd example-auth-elysia
